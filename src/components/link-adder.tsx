    ...
    <div className="space-y-3">
      <div className="flex flex-wrap gap-3">
        {links.map((l) => {
          ...
          return (
            <div key={l.id} className="group relative flex w-72 items-start gap-3 rounded-xl border bg-card p-3">
              ...
              <button onClick={() => start(() => removeProjectLink(l.id, nodeType, nodeId))} className="absolute -right-2 -top-2 hidden rounded-full bg-card p-0.5 shadow group-hover:block" aria-label="Remove"><X className="h-3.5 w-3.5" /></button>
            </div>
          );
        })}
      </div>
      <div className="flex gap-2">
        <select value={type} onChange={(e) => setType(e.target.value)} className="rounded-lg border bg-card px-2 text-sm">
