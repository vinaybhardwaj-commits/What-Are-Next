...
export function EditableTitle({ kind, id, value }: { kind: "goal" | "initiative" | "action" | "task"; id: string; value: string }) {
    ...
    return (
      <div className="flex items-center gap-2">
        <input value={v} onChange={(e) => setV(e.target.value)} autoFocus
          ...
          className="w-full rounded-lg border border-input px-2 py-1 text-2xl font-semibold text-foreground outline-none focus:ring-2 focus:ring-ring" />
  ...
  return (
    <h1 className="group flex items-center gap-2 text-2xl font-semibold text-foreground">
      <span>{value}</span>
